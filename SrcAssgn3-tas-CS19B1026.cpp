...
using namespace std;
double ma=-1;
atomic_flag lock=ATOMIC_FLAG_INIT;//intailly CS will be unlocked
...
void entry_sec(){  
  while(lock.test_and_set()){//till CS is unlocked we will wait in this loop
  } 
  //if CS is unlocked we will exit from loop
}
void exit_sec(){
 lock.clear();//CS will be unlocked
}
