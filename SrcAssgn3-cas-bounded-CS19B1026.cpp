...
void entry_sec(long long m){
 ...
 }  //this thread will wait until cs is free 
 waiting[m]=false;
}
void exit_sec(long long m){
  long long j;
  j=(m+1)%noofthreads; 
  while(j!=m && !waiting[j]) //it will select the thread which is waiting
	    j=(j+1)%noofthreads;
  if(j==m)
      lock=0; //if no thread is waiting then we will unlock cs 
  else
      waiting[j]=false;//thread m is only allowing thread j to enter the cs
}
void* testCS(void* tem){
 ...
 for(i=0;i<k;i++){
     ...
     exit_sec(id);//thread will be exited from critical section
     getTimestamp(id,i,2);
...
// ifstream ci("inp.txt");
 ...
 waiting=(bool*)malloc(sizeof(bool)*noofthreads);
